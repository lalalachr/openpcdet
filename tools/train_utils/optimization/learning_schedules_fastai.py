...
class OneCycle(LRSchedulerStep):
    def __init__(self, fai_optimizer, total_step, lr_max, moms, div_factor,
        ...
        a2 = total_step - a1
        low_lr = self.lr_max / self.div_factor                          # 学习率从low_lr->lr_max从1/10逐渐往上
        lr_phases = ((0, partial(annealing_cos, low_lr, self.lr_max)),  # annealing_cos用于将学习率从 low_lr 逐渐增加到 self.lr_max
                     (self.pct_start,                                   # 0和pct_start都代表时间，学习率开始变化的时间
                      partial(annealing_cos, self.lr_max, low_lr / 1e4)))# lr_max逐渐减小到low_lr
        mom_phases = ((0, partial(annealing_cos, *self.moms)),           # 调整动量从self.moms[0] 到 self.moms[1]
                      (self.pct_start, partial(annealing_cos,
                                               *self.moms[::-1])))       # 从 self.moms[1] 逐渐变化到 self.moms[0]
