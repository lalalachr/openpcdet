...
class KittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        # 选择模式 根据kitti_dataset.yaml中的参数
        self.split = self.dataset_cfg.DATA_SPLIT[self.mode]  
        # data/kitti/training，testing
        self.root_split_path = self.root_path / ('training' if self.split != 'test' else 'testing')
        # id路径
        split_dir = self.root_path / 'ImageSets' / (self.split + '.txt')
        # id strip去除字符串两侧的空白字符
    ...
    def include_kitti_data(self, mode):
        ...
        for info_path in self.dataset_cfg.INFO_PATH[mode]:
            ...
            with open(info_path, 'rb') as f:
                infos = pickle.load(f)
                kitti_infos.extend(infos)# 将从不同文件加载的信息（infos）合并到 kitti_infos 列表中
...
# 用于数据集划分
    ...
    def get_calib(self, idx):
        ...
        loc_lidar = np.concatenate([np.array((float(loc_obj[0]),float(loc_obj[1]),float(loc_obj[2])),dtype=np.float32).reshape(1,3) for loc_obj in idx])
        return loc_lidar
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            # R0_4x4 = np.zeros([4, 4], dtype=calib.R0.dtype) # R0 旋转矩阵 扩充为4*4
            # R0_4x4[3, 3] = 1.
            # R0_4x4[:3, :3] = calib.R0
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        dataset_cfg = EasyDict(yaml.safe_load(open(sys.argv[2])))   # argv[2] = kitti_dataset.yaml
