...
class CustomDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, ext='.bin'):
        ...
        Args:
            ...
            logger:
        """
        # dataset_cfg = custom_dataset.yaml
        super().__init__(
            dataset_cfg=dataset_cfg, class_names=class_names, training=training, root_path=root_path, logger=logger
        )
        # root_path = main函数中的data_path,即数据集存放地址
        # split = test/train(由mode)
        # root_split_path = root_path + split(由.yaml确定)
        self.split = self.dataset_cfg.DATA_SPLIT[self.mode]
        self.root_split_path = self.root_path / ('training' if self.split != 'test' else 'testing')

        # split_dir = root_path + ImageSets + split.txt，用于分割数据集地址
        split_dir = os.path.join(self.root_path,'ImageSets',(self.split + '.txt'))
        # 读取.txt数据，存进一个列表 id strip去除字符串两侧的空白字符
...
# 用于导入自定义数据
    def include_custom_data(self, mode):
        if self.logger is not None:
            self.logger.info('Loading CUSTOM dataset') # 日志信息
        custom_infos = []

         # 加载 kitti_infos_train/test.pkl到infos
        for info_path in self.dataset_cfg.INFO_PATH[mode]:
            ...
            with open(info_path, 'rb') as f:    
                infos = pickle.load(f)
                custom_infos.extend(infos)# 将从不同文件加载的信息（infos）合并到 custom_infos 列表中
...
# 用于获取雷达点云信息
    def get_lidar(self, idx):
        lidar_file = self.root_split_path / 'mylidar' / ('%s.bin' % idx)
...
# 返回标签信息
    def get_label(self, idx):
        ...
        return custom_utils.get_objects_from_label(label_file)
...
# 注释了标定信息，？
    ...
    # 将位置信息列表 loc 中的每个字符串转换为包含三个浮点数的行向量，并将所有行向量连接成一个 NumPy 数组。
    #     loc_lidar = np.concatenate([np.array((float(loc_obj[0]),float(loc_obj[1]),float(loc_obj[2])),dtype = np.float32).reshape((1,3)) for idx in loc])
    #     return loc_lidar

# 从文件中读取道路平面的参数并返回一个表示道路平面的单位法向量。？
...
# 根据点云在图像上的投影情况，确定点云是否在图像视野内 ？用于标识点云中哪些点位于相机的视场内
    # @staticmethod
 ...
 # 获取文件的信息
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            # # calib信息
            # # 垂直方向拼接 p2是左侧彩色照片 V2C雷达到相机坐标系转换 R0是矫正
            ...
            if has_label:
                ...
                # print(obj_list[0].cls_type)
                annotations = {}
                annotations['name'] = np.array([obj.cls_type for obj in obj_list])  # 类别
                # annotations['truncated'] = np.array([obj.truncation for obj in obj_list]) # 是否被截断
                # annotations['occluded'] = np.array([obj.occlusion for obj in obj_list])   # 是否被遮挡
                # annotations['alpha'] = np.array([obj.alpha for obj in obj_list])  # 物体的观察角度
                # annotations['bbox'] = np.concatenate([obj.box2d.reshape(1, 4) for obj in obj_list], axis=0)   # 2D边界框
                #annotations['dimensions'] = np.array([[obj.l, obj.h, obj.w] for obj in obj_list])  # lhw(camera) format  3D物体尺寸
                annotations['dimensions'] = np.array([[obj.l, obj.w, obj.h] for obj in obj_list])  # lhw(camera) format 
                annotations['location'] = np.concatenate([obj.loc.reshape(1, 3) for obj in obj_list], axis=0) # 3D物体位置，中心点位置
                annotations['rotation_y'] = np.array([obj.ry for obj in obj_list])  # 3D物体方向角
                annotations['score'] = np.array([obj.score for obj in obj_list])    # 置信度
                ...
                # 这里是把有效数存在一个列表前面，再加上Dontcare个空的列表个数
                # 确保索引列表的长度与标注的物体数量相同 -1代表num_gt - num_objects的列表元素的都是-1
                index = list(range(num_objects)) + [-1] * (num_gt - num_objects)
                annotations['index'] = np.array(index, dtype=np.int32) # 索引表
                ...
                #     # calib = self.get_calib(sample_idx)
                #     # pts_rect = calib.lidar_to_rect(points[:, 0:3]) # 雷达坐标系转换到相机坐标系
                #     #
                #     # fov_flag = self.get_fov_flag(pts_rect, info['image']['image_shape'], calib)
                #     # pts_fov = points[fov_flag]
                #     corners_lidar = box_utils.boxes_to_corners_3d(gt_boxes_lidar) # 顶点表示法
        ...
        with futures.ThreadPoolExecutor(num_workers) as executor:  # 创建一个线程池
            infos = executor.map(process_single_scene, sample_id_list)
        return list(infos)

# 创建真值数据库，生成gtbox的同时还建立一个存储gtbox的数据库
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        database_save_path = Path(self.root_path) / ('custom_gt_database' if split == 'train' else ('gt_database_%s' % split))
        ...
        for k in range(len(infos)):
            ...
            # 用于确定每个点是否在给定的边界框中
            # 它返回一个形状为(nboxes, npoints)的数组，表示每个点是否在每个边界框中。
            ...
            for i in range(num_obj):
                ...
                # 将点云数据中的坐标减去对应物体的真实边界框中心的坐标，以将物体置于坐标系原点 ?
...
# 用于评估
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        from .custom_object_eval_python import eval as custom_eval

        eval_det_annos = copy.deepcopy(det_annos) # 深度复制检测结果，以便进行评估。
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.custom_infos] # 深度复制每个样本的真值注释
        ap_result_str, ap_dict = custom_eval.get_official_eval_result(eval_gt_annos, eval_det_annos, class_names) # 调用自定义的评估函数
 ...
 # 用于返回训练帧的总数 数据量*训练轮数
    ...
    def __getitem__(self, index):
        # index = 4
        if self._merge_all_iters_to_one_epoch:  # 合并所有迭代器
