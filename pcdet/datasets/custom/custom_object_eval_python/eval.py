...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    '''
        print("______current_cls_name___________")
...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            # dt_boxes = np.concatenate([a["bbox"] for a in dt_annos_part if "bbox" in a], 0)
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    # total_dc_num = np.stack(total_dc_num, axis=0)
        ...
        # dt_datas_num = len(dt_annos[i]["name"])
        # dt_datas_list.append(dt_datas_num)
...
def do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges,
    ...
    mAP_bev, mAP_3d, mAP_bev_R40, mAP_3d_R40 = do_eval(gt_annos, dt_annos, current_classes, min_overlaps, compute_aos)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    overlap_0_7 = np.array([[0.7, 0.7, 0.5, 0.7,
                             0.5, 0.7], [0.7, 0.7, 0.5, 0.7, 0.5, 0.7],
